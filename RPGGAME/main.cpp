...
int main() {        // inicializa o gerador de numeros aleatorios com o tempo atual
    ...
    exibirCabecalho();     // converte time(0) para unsigned int pois srand nao aceita valores negativos
    ...
    for (int i = 0; i < qtdInimigos; i++) {
        ...
        for (int rodada = 1; rodada <= 3; rodada++) {
            if (heroi.getVida() <= 0 || goblinAtual->getVida() <= 0) break; // ponteiroverifica se o goblin ainda esta vivo

            exibirCaixaTitulo("RODADA " + std::to_string(rodada) + " De 3");

            heroi.exibirStatus();
            goblinAtual->exibirStatus(); // vida do goblin, acesso via ponteiro 
            exibirSeparador();
            ...
            //turno do goblin
            if (goblinAtual->getVida() > 0) {    // acessa o metodo pelo ponteiro goblinatual
                exibirSeparador();
                std::cout << "  --- TURNO DO GOBLIN --\n";
                goblinAtual->executarTurno(heroi); // goblin automatizado chama urno atraves do ponteiro
            }
        }
        ...
        if (heroi.getVida() <= 0) {
            exibirDerrota();
            break; 
        } else if (goblinAtual->getVida() <= 0) {    //verifica a vida do goblin pelo ponteiro
